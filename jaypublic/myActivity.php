    ...
    <div>
        <table style="">
            ...
            if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
              ?>
                <p style="margin-bottom: 25px; margin-top: 50px; font-size: 35px; font-weight: bolder; text-align: center;">내가 작성한 댓글</p>

                <?php
            }else{
                if (isset($_REQUEST['good']) && $_REQUEST['good'] == 'true'){
                    //TODO 수정필요
                    ?>
                 ...
                 ?>
                    <p style="margin-bottom: 25px; margin-top: 50px; font-size: 35px; font-weight: bolder; text-align: center;">내가 작성한 글</p>
                    <?php
                }
            }
            if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                ?>
                <tr class="table-item"><th>글번호</th><th>댓글 내용</th><th>게시판</th><th>날짜</th></tr>
...
//            $sql = "select * from $boardType order by number desc limit $start, $viewAriticle";
            ...
            if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                $sql = "select * from comment where userID = '$userID' order by date desc limit $start, $viewAriticle";
            }
...
//            }
            ...
            if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                while($row = $result->fetch_array()){
                    ...
                    $dates = explode(" ", $date);
                    $date = $dates[0];
                    $boardTypeComment = $row['boardType'];
                    if($boardTypeComment == "currentBoard"){
                        $boardTypeComment = "시사게시판";
                        $boardTypeNum = 1;
                    }else if ($boardTypeComment == "scienceBoard"){
                        $boardTypeComment = "IT/과학게시판";
                        $boardTypeNum = 2;
                    }else if ($boardTypeComment == "freeBoard"){
                        $boardTypeComment = "자유게시판";
                        $boardTypeNum = 3;
                    }
                    $bnum = $row['boardNumber'];
                    ?>
                    <tr><td><?php echo $bnum;?></td><td onclick="location.href='read.php?number=<?php echo $bnum;?>&board=<?php echo $boardTypeNum;?>'"><div class="table-title"><p><?php echo $content;?></p></div></td><td><?php echo $boardTypeComment;?></td><td><?php echo $date;?></td></tr>
                    <?php
                }
            }else{
                while($row = $result->fetch_array()){
        ...
        <div style=" width: fit-content; margin: auto; margin-top: 100px;">
            ...
            if($page != 1){
                if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                    echo "<a style='margin: 0px 10px 0px 10px; font-weight: bolder; font-size: 25px; color: black' href='myActivity.php?page=1&comment=true'>처음</a>";

                }else{
                    echo "<a style='margin: 0px 10px 0px 10px; font-weight: bolder; font-size: 25px; color: black' href='myActivity.php?page=1'>처음</a>";
                }
            } else {
            ...
            if($startPage != 1){
                if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                    echo "<a style='margin: 0px 10px 0px 10px; color: black; font-size: 25px' href='myActivity.php?page=$prevBlock&comment=true'>◀︎</a>";

                }else{
                    echo "<a style='margin: 0px 10px 0px 10px; color: black; font-size: 25px' href='myActivity.php?page=$prevBlock'>◀︎</a>";
                }
            }
            ...
            for($i = $startPage; $i<$endPage; $i++){
                ...
                if($i == $page) {
                    ...
                    if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                        echo "<a style='margin: 0px 10px 0px 10px; color: black; font-weight: bolder; font-size: 25px;' href='myActivity.php?page=$i&comment=true'>$i</a>";
                    }else{
                        echo "<a style='margin: 0px 10px 0px 10px; color: black; font-weight: bolder; font-size: 25px;' href='myActivity.php?page=$i'>$i</a>";
                    }
                }
            }
            //다음 블록 이동.
            ...
            if($nextBlock < $totalPage){
                if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                    echo "<a style='margin: 0px 10px 0px 10px; color: black; font-size: 25px' href='myActivity.php?page=$nextBlock&comment=true'>▶︎︎</a>";
                }else {
                    echo "<a style='margin: 0px 10px 0px 10px; color: black; font-size: 25px' href='myActivity.php?page=$nextBlock'>▶︎︎</a>";
                }
            }
            ...
            if($page != $totalPage){
                if (isset($_REQUEST['comment']) && $_REQUEST['comment'] == 'true'){
                    echo "<a style='margin: 0px 10px 0px 10px; font-weight: bolder; font-size: 25px; color: black' href='myActivity.php?page=$totalPage&comment=true'>끝</a>";
                }else{
                    echo "<a style='margin: 0px 10px 0px 10px; font-weight: bolder; font-size: 25px; color: black' href='myActivity.php?page=$totalPage'>끝</a>";
                }
            } else {
